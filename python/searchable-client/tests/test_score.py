...
def test_avg_field_length_zero_is_respected():
    ...
    manifest_with_zero_avg = Manifest(
        version=1, build_id="x", format="json", languages=["en"], default_language="en",
        fields={"title": FieldConfig(boost=2.0, stored=True)},
        doc_count={"en": 10}, avg_field_length={"en": {"title": 0.0}},
        shards_terms=[], shards_docs=[],
    )
    ...
    manifest_no_avg = Manifest(
        version=1, build_id="x", format="json", languages=["en"], default_language="en",
        fields={"title": FieldConfig(boost=2.0, stored=True)},
        doc_count={"en": 10}, avg_field_length={"en": {}},
        shards_terms=[], shards_docs=[],
    )
    posting = Posting(doc=1, fields={"title": FieldPosting(tf=1, pos=[0], len=4)})
    ...
    score_without_avg = score_term_for_doc(
        posting, df=2, manifest=manifest_no_avg, language="en"
    )
