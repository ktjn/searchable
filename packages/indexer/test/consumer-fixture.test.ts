 ...
 * `exports` field and resolves "@ktjn/searchable-indexer" as this package's own
...
describe("package.json exports/bin (consumer-fixture)", () => {
  ...
  it("declares a bin entry whose target file exists after build", async () => {
    const mainEntryPath = require.resolve("@ktjn/searchable-indexer");
  ...
  it("runs as a real executable script via node, indexing a directory end-to-end", async () => {
    ...
    const inputDir = await mkdtemp(join(tmpdir(), "searchable-cli-in-"));
    const outDir = await mkdtemp(join(tmpdir(), "searchable-cli-out-"));
