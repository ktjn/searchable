...
Document-store shards have two compatible versions. Legacy unstructured indexes use
v1. Structured indexes produced by `build_index_documents()` can opt into v2 with
`doc_store_format="binary"`; the manifest marks those shards with
`binaryVersion: 2`. V2 preserves the document URL, boost, external ID, content
hash, JSON-compatible metadata, and stored string fields. Python and TypeScript
clients reject unsupported versions, invalid magic bytes, truncated records, and
