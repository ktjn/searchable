...
- [x] **F.1** (old 6.1) Single home for the binary-format codec. Implemented for
  Python: the codec was split across `searchable-indexer` (encode) and
  `searchable-client` (decode), plus a third hand-rolled copy in the client's
  test fixtures. A new neutral `searchable-binary` package now houses the byte
  reader/writer + per-shard encode **and** decode (`term_shard.py`,
  `fuzzy_shard.py`, `doc_store.py`) and the binary container types; the indexer
  and client both depend on it, and the client fixtures use its encoders
  (dropping the ~170-line hand-rolled block). The TS-side decode stays in
